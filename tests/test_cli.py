...
def test_info_command(monkeypatch):
    ...
    monkeypatch.setattr("odctl.workspace.get_cli_version", lambda: "0.0.3")
    monkeypatch.setattr("odctl.docker.is_docker_running", lambda: True)

    result = runner.invoke(app, ["info"])
    assert result.exit_code == 0
    assert "Open Data Stack" in result.stdout
...
def test_up_command_dry_run(monkeypatch, mock_registry_data):
    ...
    monkeypatch.setattr("odctl.docker.launch_stack", mock_launch)
